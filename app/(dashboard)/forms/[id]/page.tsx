...
import { GetFormById, GetFormWithSubmissions } from '@/app/lib/actions/form';
        ...
        <TableBody>
          {
            rows.map((row, index) => (
              <TableRow key={index}>{
                columns.map(column => (
                  <RowCell key={column.id} type={column.type} value={row[column.id]}></RowCell>
            ))}
              <TableCell className="text-right">{formatDistance(row.submittedAt, new Date(), {
                addSuffix: true
              })}</TableCell>
                </TableRow>
            ))
